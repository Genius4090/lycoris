  ...
  if (cartItems.length === 0) {
    return (
      <section className="containers min-h-screen flex flex-col items-center justify-center gap-6">
        ...
        <h2 className="font-liter text-3xl text-title">Nothing to check out</h2>
        <p className="text-textish font-liter text-sm">Add items to your cart first.</p>
    ...
    <section className="containers min-h-screen pt-36 pb-24">
      <div className="flex flex-col lg:flex-row gap-10 items-start justify-center">
        ...
        <div className="flex-1 max-w-xl">
          <h1 className="font-liter text-4xl text-title mb-2">Review Order</h1>
          <p className="font-liter text-sm text-textish mb-8">
            {totalItems} {totalItems === 1 ? "item" : "items"} — confirm before placing
          </p>
            ...
            {cartItems.map((item) => (
              <li key={item.id} className="flex items-center gap-4 py-5">
                ...
                <div className="flex-1 min-w-0">
                  <p className="font-liter text-title text-base leading-snug truncate">
                    {item.product.title}
                  </p>
        ...
        <aside className="w-full lg:w-72 shrink-0 lg:sticky lg:top-28">
          <div className="bg-brownish/30 border border-brownish p-6 flex flex-col gap-5">
            <h2 className="font-liter text-xl text-title">Payment Summary</h2>

            <div className="flex flex-col gap-2 border-t border-brownish/60 pt-4">
              <div className="flex justify-between text-sm">
                <span className="font-liter text-textish">Subtotal</span>
                <span className="font-liter text-title">{total.toFixed(2)} €</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="font-liter text-textish">Delivery</span>
                <span className="font-liter text-title">Free</span>
              </div>
            </div>

            <div className="flex justify-between items-center border-t border-brownish pt-4">
              <span className="font-liter text-title text-base">Total</span>
              <span className="font-liter text-title text-xl font-medium">
                {total.toFixed(2)} €
              </span>
            </div>

            {checkoutMutation.isError && (
              <p className="text-pinkish text-xs font-liter text-center">
            ...
            <div className="border border-brownish/80 p-2">
              <button
                ...
                className="cursor-pointer w-full font-liter flex justify-center items-center gap-2 text-title bg-brownish py-2.5 px-7 hover:bg-brownish/70 transition-colors disabled:opacity-50"
              >
            ...
            <p className="text-xs font-liter text-textish text-center leading-relaxed">
